...
@click.option('-e', '--environment', default='prod', type=click.File(), help='Environment namespace to use for SSM variables')
@click.option('-v', '--version', is_flag=True, callback=print_version, expose_value=False, is_eager=True, help='Print version and exit.')
def barbara_deploy(skip_existing, destination, template):
